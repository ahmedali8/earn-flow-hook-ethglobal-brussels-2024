...
contract RewardsDistributor is Ownable {
    ...
    /// @param _owner Contract owner, can call functions that change state.
    ...
    function deposit(address staker, uint256 tokens) public onlyOwner returns (bool success) {
        uint256 _tokensToAdd = tokens + (_stakeRemainder[staker]);

        uint256 _eligibleUnitsToAdd = _tokensToAdd / ELIGIBLE_UNIT;

        // update the new remainder for this address
        _stakeRemainder[staker] = _tokensToAdd % ELIGIBLE_UNIT;

        // set the current stake for this address
        _stake[staker] = _stake[staker] + (_eligibleUnitsToAdd);

        // update total eligible stake units
        _stakeTotal = _stakeTotal + (_eligibleUnitsToAdd);
    ...
    function distribute(address from, uint256 tokens) public onlyOwner returns (bool success) {
        ...
        uint256 _amountToDistribute = tokens + (_rewardRemainder);

        if (_stakeTotal == 0) {
            _rewardRemainder = _amountToDistribute;
        } else {
            // determine rewards per eligible stake
            uint256 _ratio = _amountToDistribute / _stakeTotal;

            // carry on remainder
            _rewardRemainder = _amountToDistribute % _stakeTotal;

            // increase total rewards per stake unit
            _rewardTotal = _rewardTotal + (_ratio);
        }
    ...
    function withdrawReward(address staker) public onlyOwner returns (uint256 tokens) {
        ...
        _rewardOffset[staker] = (int256)(_rewardTotal * _stake[staker]);
    ...
    function withdrawStake(address staker, uint256 tokens) public onlyOwner returns (bool) {
        ...
        uint256 _newStake = _currentStake - tokens;

        _stakeRemainder[staker] = _newStake % ELIGIBLE_UNIT;

        uint256 _eligibleUnitsDelta = _stake[staker] - _newStake / ELIGIBLE_UNIT;

        _stake[staker] = _stake[staker] - _eligibleUnitsDelta;

        // update total stake
        _stakeTotal = _stakeTotal - _eligibleUnitsDelta;

        // update reward offset
        _rewardOffset[staker] -= int256(_rewardTotal * _eligibleUnitsDelta);
    ...
    function getReward(address staker) public view returns (uint256 tokens) {
        int256 _tokens = int256(_stake[staker] * _rewardTotal) - _rewardOffset[staker];
